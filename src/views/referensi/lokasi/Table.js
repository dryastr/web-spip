...
import TableHeader from 'src/views/referensi/lokasi/TableHeader'
import OptionActions from 'src/views/referensi/lokasi/components/OptionActions'
...
const defaultColumns = [
  ...
  },
  {
    ...
    minWidth: 150,
    headerName: 'Level',
    renderCell: ({ row }) => <Typography fontSize={14}>{row.level}</Typography>
  },
  {
    flex: 0.25,
    minWidth: 215,
...
const Table = () => {
  ...
  const loadData = params => {
    setIsLoading(true)
    setParams(params)
    lokasiService.getPagination(params).then(response => {
  ...
  const handleDialogDelete = () => {
    ...
    lokasiService.delete(item.pid).then(response => {
