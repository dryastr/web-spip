...
const FormInput = props => {
  ...
  useEffect(() => {
    if (props.action && props.action === 'edit') {
      jenisKkLead.show(props.id).then(response => {
        reset(response)
      })
    }
  }, [])
  ...
  const onSubmit = async data => {
    ...
    if (props.action && props.action === 'edit') {
      jenisKkLead.update(props.id, data).then(response => {
        dispatch(hideLoading())
        if (response?.status === 200) {
          reset()
          toast.success('Data Jenis KK Lead SPIP berhasil diubah.', styleToast)
          Router.push('/referensi/jenis-kk-lead-spip')
        } else {
          toast.error('Data Jenis KK Lead SPIP gagal diubah.', styleToastError)
        }
      })
    } else {
      jenisKkLead.store(data).then(response => {
        dispatch(hideLoading())
        if (response?.status === 200) {
          reset()
          toast.success('Data Jenis KK Lead SPIP berhasil ditambahkan.', styleToast)
          Router.push('/referensi/jenis-kk-lead-spip')
        } else {
          toast.error('Data Jenis KK Lead SPIP gagal ditambahkan.', styleToastError)
        }
      })
    }
  }
      ...
      <form noValidate autoComplete='off' onSubmit={handleSubmit(onSubmit)}>
        ...
        <CardActions>
          <Button
            sx={{ marginRight: 2 }}
            component={Link}
            href='/referensi/jenis-kk-lead-spip'
            type='button'
            size='large'
            color='error'
            variant='contained'
