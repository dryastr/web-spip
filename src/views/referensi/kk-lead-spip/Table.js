...
import TableHeader from 'src/views/referensi/kk-lead-spip/TableHeader'
import OptionActions from 'src/views/referensi/kk-lead-spip/components/OptionActions'
...
const defaultColumns = [
  ...
  },
  {
    ...
    field: 'nama',
    headerName: 'Nama',
    renderCell: ({ row }) => <Typography variant='body2'>{row.nama}</Typography>
  },
  {
    flex: 0.35,
    minWidth: 215,
    field: 'bobot',
    headerName: 'Bobot',
    renderCell: ({ row }) => <Typography variant='body2'>{row.bobot}</Typography>
  },
  {
    flex: 0.35,
    minWidth: 215,
    field: 'jenis_lead',
    headerName: 'Jenis KK Lead',
    renderCell: ({ row }) => <Typography variant='body2'>{row.jenis_kk_lead_spip?.nama || '-'}</Typography>
  }
]
...
const Table = () => {
  ...
  const loadData = params => {
    setParams(params)
    kkleadserviceService.getKk(params).then(response => {
  ...
  const handleDialogDelete = () => {
    ...
    kkleadserviceService.delete(item.pid).then(response => {
