...
const FormInput = props => {
  ...
  useEffect(() => {
    ...
    if (props.action && props.action === 'edit') {
      jenisSasaran.show(props.id).then(response => {
  ...
  const onSubmit = async data => {
    ...
    if (props.action && props.action === 'edit') {
      jenisSasaran.update(props.id, data).then(response => {
        dispatch(hideLoading())
        if (response?.status === 200) {
          reset()
          toast.success('Data Jenis Sasaran berhasil diubah.', styleToast)
          Router.push('/referensi/jenis-sasaran')
        } else {
          toast.error('Data Jenis Sasaran gagal diubah.', styleToastError)
        }
      })
    } else {
      jenisSasaran.store(data).then(response => {
        dispatch(hideLoading())
        if (response?.status === 200) {
          reset()
          toast.success('Data Jenis Sasaran berhasil ditambahkan.', styleToast)
          Router.push('/referensi/jenis-sasaran')
        } else {
          toast.error('Data Jenis Sasaran gagal ditambahkan.', styleToastError)
        }
      })
    }
  }
